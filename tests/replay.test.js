...
test("raw to transform to evidence to replay is deterministic",()=>{
 ...
 const transform=createTransform({transformId:"status-v1",version:"1",apply(bytes)=>{
 ...
 try{
  const bundle=persistRecord(db,{executionId:"replay-exec",source:"fixture",adapter:"transform-test",adapterVersion:"1",step:"status",data:first.data,observedAt:"2026-09-01T00:00:00.000Z",rawArtifact:raw,rawMediaType:"text/plain",transformId:transform.id});
