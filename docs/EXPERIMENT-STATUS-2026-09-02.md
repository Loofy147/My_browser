...
- Identity replay is explicitly treated as separate from source-to-output transformation replay.
...
The local execution environment cannot resolve external GitHub/registry hosts, so local clean-install or CI reproduction is not asserted from the local environment.

The current gate also does not prove committed-lock reproducibility because `package-lock.json` is not committed.

## Decision

**Status: GATE PASSED / INTERNAL CAPABILITY CANDIDATE.**

The kernel abstraction survived the current acceptance gate without requiring source-specific semantic exceptions within the tested scope.

Do not turn it into a browser product or generic scraping product on the basis of this result.

The next controlled experiment is limited to a small number of additional source shapes and, more importantly, measurement of whether canonical Evidence materially improves research, verification, or decision workflows. Additional implementation must be justified by observed downstream value, not by feature completeness.
